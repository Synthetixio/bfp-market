...
describe('PerpMarketFactoryModule Utilization', () => {
  ...
  describe('Market digest UtilizationRate', () => {
    ...
    it('can handle utilization more than 100%', async () => {
      ...
      const { minDelegationD18 } = await Core.getCollateralConfiguration(stakedCollateral().address);
      ...
      const { collateral: collateral1, collateralDepositAmount: collateralDepositAmount1 } = await depositMargin(
        bs,
        genTrader(bs, { desiredMarket: market, desiredTrader: trader1 })
      );
      ...
      const { collateral: collateral2, collateralDepositAmount: collateralDepositAmount2 } = await depositMargin(
        bs,
        genTrader(bs, { desiredMarket: market, desiredTrader: trader2 })
      );
      ...
      const recomputeUtilizationEvent1 = findEventSafe(recomputeReceipt1, 'UtilizationRecomputed', PerpMarketProxy);
      ...
      const recomputeUtilizationEvent2 = findEventSafe(recomputeReceipt2, 'UtilizationRecomputed', PerpMarketProxy);
